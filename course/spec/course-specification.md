...
|   |   |   |   |   |   |   |   |
| Session 5, LLM half — FIRST INCREMENT WRITTEN. Lab 5 Part B (Tasks 7 to 9) covers the evaluation gate, guardrails, and token cost, backed by src/llmcost.py, scripts/llm_eval.py, a worked golden set under evals/, and tests. STILL TO WRITE: the Session 5 slide deck for this half, Drill 5 questions on it, and a live managed-LLM-endpoint walkthrough — the lab runs on recorded responses so it stays free and offline. |   |   |   |   |   |   |   |
